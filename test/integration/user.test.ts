...
describe('Users API', () => {
  ...
  let patientUserToken = null;
  const patientUser = new User();
  patientUser.username = 'Toddy';
  patientUser.name = 'Todd Alquist';
  patientUser.email = 'todd.alquist@test.com';
  patientUser.password = userPassword;
  patientUser.hashPassword();
  patientUser.role = 'PATIENT' as Role;
  ...
  beforeEach(async () => {
    await userRepository.save([adminUser, patientUser]);
    let res = await request(app).post('/v1/login').send({ email: adminUser.email, password: userPassword });
    adminUserToken = res.body.data;
    res = await request(app).post('/v1/login').send({ email: patientUser.email, password: userPassword });
    patientUserToken = res.body.data;
  });

  afterEach(async () => {
    await userRepository.delete([adminUser.id, patientUser.id]);
  });
  ...
  describe('GET /v1/users', () => {
    ...
    it('should report error of unauthorized user', async () => {
      const res = await request(app).get('/v1/users').set('Authorization', patientUserToken);
      ...
      expect(res.body.errors).to.eql([
        ...
        'Current role: patient. Required role: ADMINISTRATOR',
      ]);
